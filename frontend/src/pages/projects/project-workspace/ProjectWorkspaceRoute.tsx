...
import { ChapterEditorPanel } from "../../../features/edit-chapter/ui/ChapterEditorPanel";
import { useWorkspaceDraftStore } from "../../../features/edit-project-workspace";
...
export function ProjectWorkspaceRoute({ projectId, section }: ProjectWorkspaceRouteProps) {
  ...
  const updateWorkspaceMutation = useMutation({
    ...projectMutations.updateWorkspace(projectId),
    onSuccess: async (saved) => {
      setWorkspaceFromServer(saved);
      await Promise.all([
  ...
  const continuityMutation = useMutation(projectMutations.continuityCheck(projectId));
  const workspace = useWorkspaceDraftStore((state) => state.workspace);
  const savedWorkspace = useWorkspaceDraftStore((state) => state.savedWorkspace);
  const setWorkspace = useWorkspaceDraftStore((state) => state.setWorkspace);
  const setSavedWorkspace = useWorkspaceDraftStore((state) => state.setSavedWorkspace);
  const setWorkspaceFromServer = useWorkspaceDraftStore((state) => state.setWorkspaceFromServer);
  const continuityText = useWorkspaceDraftStore((state) => state.continuityText);
  const setContinuityText = useWorkspaceDraftStore((state) => state.setContinuityText);
  const providers = useMemo(() => providersQuery.data || [], [providersQuery.data]);
  const [error, setError] = useState<string>();
  const [notice, setNotice] = useState<string>();
  const [continuityCheck, setContinuityCheck] = useState<ContinuityCheck>();
  const loading = workspaceQuery.isPending || providersQuery.isPending;
  const saving = updateWorkspaceMutation.isPending;
  const checkingCanon = continuityMutation.isPending;
  const queryError =
  ...
  useEffect(() => {
    if (workspaceQuery.data) {
      setWorkspaceFromServer(workspaceQuery.data);
      return;
    }
  ...
  async function runContinuityCheck() {
    ...
    setNotice(undefined);
    try {
      const check = await continuityMutation.mutateAsync({
        text: continuityText,
        provider_id: workspace.project.provider_id,
        model_id: workspace.project.model_id,
      });
  ...
  async function saveCurrentSection() {
    ...
    try {
      await updateWorkspaceMutation.mutateAsync(workspacePayload(workspace));
  ...
  if (!workspace) {
    return (
      <AppShell>
        <div className="workspace-route__state is-error">
          {displayError || "Workspace not found."}
        </div>
      </AppShell>
    );
  }
  ...
  return (
    <AppShell>
      <div className="workspace-route">
        ...
        {(displayError || notice) && (
          <div className={`workspace-route__message ${displayError ? "is-error" : "is-ready"}`}>
            {displayError || notice}
          </div>
        )}
