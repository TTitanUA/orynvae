...
type CreateField = keyof ProjectCreateDraft | "ideaText" | "selectedProviderId" | "selectedModelId";
...
export function ProjectCreateRoute() {
  ...
  const analyzeMutation = useMutation(projectMutations.analyzeSetup());
  const createMutation = useMutation({
    ...projectMutations.createFromSetup(),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: projectQueryKeys.list() });
    },
  });
  const providers = useMemo(() => providersQuery.data || [], [providersQuery.data]);
  const ideaText = useCreateProjectStore((state) => state.ideaText);
  const setIdeaText = useCreateProjectStore((state) => state.setIdeaText);
  const selectedProviderId = useCreateProjectStore((state) => state.selectedProviderId);
  const setSelectedProviderId = useCreateProjectStore((state) => state.setSelectedProviderId);
  const selectedModelId = useCreateProjectStore((state) => state.selectedModelId);
  const setSelectedModelId = useCreateProjectStore((state) => state.setSelectedModelId);
  const draft = useCreateProjectStore((state) => state.draft);
  const updateDraftFieldInStore = useCreateProjectStore((state) => state.updateDraftField);
  const applyAnalysis = useCreateProjectStore((state) => state.applyAnalysis);
  const [error, setError] = useState<string>();
  const [notice, setNotice] = useState<string>();
  const [warnings, setWarnings] = useState<string[]>([]);
  const [createdProjectId, setCreatedProjectId] = useState<string>();
  const [fieldErrors, setFieldErrors] = useState<FieldErrors>({});
  const loadingProviders = providersQuery.isPending;
  const analyzing = analyzeMutation.isPending;
  const saving = createMutation.isPending;
  const queryError = providersQuery.error instanceof Error ? providersQuery.error.message : undefined;
  ...
  useEffect(() => {
    ...
    const firstReadyProvider = preferredProvider(providers);
  ...
  function updateDraftField<K extends keyof ProjectCreateDraft>(
    field: K,
    value: ProjectCreateDraft[K],
  ) {
    updateDraftFieldInStore(field, value);
  ...
  return (
    <AppShell>
      <div className="project-create-route">
        ...
        {(displayError || notice) && (
          <div className={`project-create-route__message ${displayError ? "is-error" : "is-ready"}`}>
            <span>{displayError || notice}</span>
            {createdProjectId && !displayError && (
