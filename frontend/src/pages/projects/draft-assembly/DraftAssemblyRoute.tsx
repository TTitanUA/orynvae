...
export function DraftAssemblyRoute({ projectId, sessionId }: DraftAssemblyRouteProps) {
  ...
  const canAssemble = Boolean(canMutate && detail?.session.status && ["completed", "draft_ready"].includes(detail.session.status));
  ...
  function requestAssist() {
    if (!canMutate || !chapterId || !selectionMarkdown.trim() || !assistInstructions.trim()) {
      return;
    }
            ...
            <main className="draft-workspace">
              ...
              <section className="draft-panel draft-assist">
                ...
                <div className="draft-actions-row">
                  <button
                    disabled={!canMutate || assistMutation.isPending || !selectionMarkdown.trim() || !assistInstructions.trim()}
