...
import {
  ...
  type OpenRouterSortMode,
  type Provider,
  type ProviderCreatePayload,
  type ProviderDefaults,
  type ProviderModel,
  type ProviderModelPreferencesUpdatePayload,
  type ProviderType,
} from "../../../entities/provider";
import { initialProviderForm, useProviderFormStore } from "../../../features/manage-providers";
...
const initialForm: ProviderFormState = initialProviderForm;
...
export function ProviderSettingsRoute() {
  ...
  const defaultsQuery = useQuery(providerQueries.defaults());
  const providersQuery = useQuery(providerQueries.list());
  const createProviderMutation = useMutation(providerMutations.create());
  const updateProviderMutation = useMutation(providerMutations.update());
  const deleteProviderMutation = useMutation(providerMutations.delete());
  const testProviderMutation = useMutation(providerMutations.test());
  const refreshProviderModelsMutation = useMutation(providerMutations.refreshModels());
  const setDefaultProviderMutation = useMutation(providerMutations.setDefaultProvider());
  const updateModelPreferencesMutation = useMutation(providerMutations.updateModelPreferences());
  const defaults = useMemo(() => defaultsQuery.data || [], [defaultsQuery.data]);
  const providers = useMemo(() => providersQuery.data || [], [providersQuery.data]);
  const form = useProviderFormStore((state) => state.form);
  const setForm = useProviderFormStore((state) => state.setForm);
  ...
  const loading = defaultsQuery.isPending || providersQuery.isPending;
  const saving = createProviderMutation.isPending;
  const queryError =
  ...
  async function invalidateProviderState() {
    await queryClient.invalidateQueries({ queryKey: providerQueryKeys.all });
  }

  useEffect(() => {
    if (!defaults.some((item) => item.type === form.type) && defaults[0]) {
      setForm(formFromDefaults(defaults[0]));
    }
  }, [defaults, form.type, setForm]);

  function preferenceForProvider(provider: Provider): ProviderPreferenceDraft {
    return preferenceFromProvider(provider, modelPreferences[provider.id]);
  }
  ...
  function updatePreference(
    ...
    setModelPreferences((current) => ({
      ...current,
      [provider.id]: updater(preferenceFromProvider(provider, current[provider.id])),
    }));
  }
  ...
  function updateSort(provider: Provider, modelId: string, mode: OpenRouterSortMode | "") {
    const state = preferenceForProvider(provider);
  ...
  async function submitProvider(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const provider = await createProviderMutation.mutateAsync(payload);
  ...
  async function saveModelPreferences(provider: Provider) {
    const draft = preferenceForProvider(provider);
    ...
    setNotice(undefined);
    try {
      await updateModelPreferencesMutation.mutateAsync({ providerId: provider.id, payload });
  ...
  async function runProviderAction(
    ...
    try {
      ...
      if (action === "refresh") {
        const result = await refreshProviderModelsMutation.mutateAsync(provider.id);
      ...
      if (action === "default") {
        await setDefaultProviderMutation.mutateAsync(provider.id);
  ...
  return (
    <AppShell>
      <div className="provider-route">
        ...
        {(displayError || notice) && (
          <div className={`provider-route__message ${displayError ? "is-error" : "is-ready"}`}>
            {displayError || notice}
          </div>
        )}
        ...
        <section className="provider-route__layout">
          ...
          <section className="provider-list" aria-label="Настроенные провайдеры">
            ...
            {providers.map((provider) => {
              const preference = preferenceForProvider(provider);
              const allowedCount = provider.models.filter(
